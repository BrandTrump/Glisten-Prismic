    ...
    >
      <PrismicRichText
        ...
        components={{
          ...
          em: ({ children }) => (
            <em className="bg-yellow-500 bg-gradient-to-b from-yellow-100 bg-clip-text not-italic text-transparent">
              {children}
            </em>
          ),
        }}
      ...
      <div className="mt-16 grid max-w-4xl grid-rows-[auto_auto_auto] gap-8 md:grid-cols-3 md:gap-10">
        {slice.items.map((item) => (
          ...
          >
            ...
            <PrismicNextImage field={item.image} className="max-h-36 w-full" />
          </div>
        ))}
      </div>
    </Bounded>
